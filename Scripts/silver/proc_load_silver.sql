...
BEGIN
	...
	BEGIN TRY
		...
		*,
		...
		SELECT  
			prd_id,
			REPLACE(SUBSTRING(prd_key,1, 5), '-', '_') AS cat_id, -- Extract Category ID (split prd_key) 
			SUBSTRING(prd_key, 7, LEN(prd_key)) AS prd_key, -- Extract Prduct Key
			prd_nm,
		...
		SET @start_time = GETDATE();
		PRINT'>> Truncating the Table:silver_erp_cust_az12';
